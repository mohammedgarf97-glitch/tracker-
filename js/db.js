// ===== قاعدة البيانات المحلية (تحاكي Supabase) =====
// عند الرفع الفعلي، استبدل هذا الملف بـ Supabase client
...
const DB = {
  ...
  init() {
    if (!localStorage.getItem('db_initialized')) {
      ...
      this.set('announcements', [
        { id: 1, title: 'تذكير موعد الامتحان الشهري', body: 'يُذكَّر جميع الطلاب بأن الامتحان الشهري سيكون يوم الثلاثاء القادم في تمام الساعة 4 مساءً. يُرجى المراجعة جيداً ومراجعة الفصول من 1 إلى 5.', date: '15 يونيو 2025', target: 'الكل', likes: 12, likedBy: [] },
        { id: 2, title: 'إضافة ملخصات الفصل الثالث', body: 'تم رفع ملخصات الفصل الثالث في قسم الموارد التعليمية لمجموعة أ ومجموعة ب. يمكنكم الآن تحميلها من قسم الموارد.', date: '12 يونيو 2025', target: 'مجموعة أ', likes: 8, likedBy: [] },
        { id: 3, title: 'تغيير موعد حصة الخميس', body: 'سيتم تغيير موعد حصة يوم الخميس لتكون الساعة 5 مساءً بدلاً من الساعة 4 مساءً لهذا الأسبوع فقط.', date: '10 يونيو 2025', target: 'الكل', likes: 5, likedBy: [] },
      ]);
  ...
  getStudents() {
    return this.get('users').filter(u => u.role === 'student');
  },

  getStudentById(id) {
    return this.get('users').find(u => u.id === parseInt(id));
  },
  ...
  deleteStudent(id) {
    const users = this.get('users').filter(u => u.id !== parseInt(id));
    this.set('users', users);
  },
  ...
  deleteAnnouncement(id) {
    const anns = this.get('announcements').filter(a => a.id !== parseInt(id));
    this.set('announcements', anns);
  },
  ...
  deleteResource(id) {
    const res = this.get('resources').filter(r => r.id !== parseInt(id));
    this.set('resources', res);
  },
  ...
  toggleAttendance(studentId, date) {
    ...
    if (idx !== -1) {
      ...
      att[idx].time = att[idx].present ? new Date().toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' }) : null;
    } else {
      att.push({ id: Date.now(), studentId: parseInt(studentId), date, present: true, time: new Date().toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' }) });
    }
    this.set('attendance', att);
    if (idx !== -1) return att[idx];
    return att[att.length - 1];
  },

  markAttendanceByQR(studentId, code) {
    const session = this.get('session_qr');
    if (!session || session.code !== code) return false;
    const today = new Date().toISOString().split('T')[0];
    this.toggleAttendance(studentId, today);
    return true;
  },
  ...
  togglePayment(studentId) {
    ...
    if (idx !== -1) {
      users[idx].paid = !users[idx].paid;
      this.set('users', users);
      return users[idx].paid;
    }
  },
};
